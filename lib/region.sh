...
# pre-scan: member-list indent per group (the first '- ' line under each
# group's proxies:). Some configs (e.g. generated by sub conversion) indent
# members two spaces deeper than the proxies: line itself; injecting refs at
# the proxies: indent then mixes sequence indents, which YAML rejects.
member_indent = {}
cur, seen_proxies = None, False
for line in lines:
    m = re.match(r'^\s*-\s+name:\s*(.+?)\s*$', line)
    if m:
        cur, seen_proxies = m.group(1).strip("'\""), False
        continue
    if cur and re.match(r'^\s+proxies:\s*$', line):
        seen_proxies = True
        continue
    if cur and seen_proxies and cur not in member_indent:
        m = re.match(r'^(\s*)-\s+', line)
        if m:
            member_indent[cur] = len(m.group(1))
            cur, seen_proxies = None, False
for line in lines:
    ...
    if in_groups:
        ...
        if sel and block and not inserted_refs and cur_group == sel and re.match(r'^\s+proxies:\s*$', line):
            out.append(line)
            n = member_indent.get(sel, len(re.match(r'^(\s*)', line).group(1)) + 2)
            pad = ' ' * n
